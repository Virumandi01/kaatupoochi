...
export default function Home() {
  ...
  const handleFetch = async (platform: string, repoDetails: string) => {
    ...
    try {
      ...
      // Get full year ACTIVITY (Fast Load)
      const res = await axios.get(
        `http://localhost:3000/api/github/commits/${ownerPart}/${repoPart}/activity/${latestYear}?branch=${infoRes.data.default_branch}`
      );
      setActivityMap(res.data.activity);
      
      // Calculate total commits from activity map
  ...
  const handleDayClick = async (date: string) => {
    ...
    // Fetch commits specifically for this day
    try {
      const res = await axios.get(
        `http://localhost:3000/api/github/commits/${owner}/${repo}/day/${date}?branch=${selectedBranch}`
      );
      setSelectedCommits(res.data.commits);
    // eslint-disable-next-line @typescript-eslint/no-unused-vars
    } catch (_err) {
      alert('Could not load commits for this day.');
    }
  };
  ...
  const handleBranchChange = async (branchName: string) => {
    ...
    try {
      const res = await axios.get(
        `http://localhost:3000/api/github/commits/${owner}/${repo}/activity/${selectedYear}?branch=${branchName}`
      );
      setActivityMap(res.data.activity);
      const total = Object.values(res.data.activity).reduce((sum: any, count: any) => sum + count, 0);
      setTotalCommits(total as number);
  ...
  const handleYearChange = async (year: number) => {
    ...
    try {
      const res = await axios.get(
        `http://localhost:3000/api/github/commits/${owner}/${repo}/activity/${year}?branch=${selectedBranch}`
      );
      setActivityMap(res.data.activity);
      const total = Object.values(res.data.activity).reduce((sum: any, count: any) => sum + count, 0);
      setTotalCommits(total as number);
