...
router.get('/:owner/:repo/repoinfo', async (req, res) => {
  try {
    ...
    const years = [];
    for (let y = currentYear; y >= createdYear; y--) years.push(y);
    const latestCommitRes = await octokit.repos.listCommits({ owner, repo, per_page: 1 });
...
router.get('/:owner/:repo/readme', async (req, res) => {
  try {
    ...
    res.json({ success: true, content, name: data.name });
  } catch (error) {
...
// 4. activity — only dates and counts for a year (FAST)
router.get('/:owner/:repo/activity/:year', async (req, res) => {
  try {
    ...
    res.json({ success: true, year, activity });
...
// 5. day commits — load commits for a specific day
router.get('/:owner/:repo/day/:date', async (req, res) => {
  try {
    ...
    res.json({ success: true, date, commits });

  } catch (error) {
    console.error('Day commits error:', error.message);
    res.status(500).json({ success: false, error: error.message });
  }
});
  ...
  try {
    ...
    res.json({
      ...
      commit: {
        ...
        github_url: `https://github.com/${owner}/${repo}/commit/${data.sha}`,
      }
    });
  } catch (error) {
...
// 8. LAST — basic commits list
router.get('/:owner/:repo', async (req, res) => {
  try {
    const { owner, repo } = req.params;
    const { limit = 10 } = req.query;
    const { data: commits } = await octokit.repos.listCommits({ owner, repo, per_page: limit });
    ...
    res.json({ success: true, platform: 'github', owner, repo, total: cleanCommits.length, commits: cleanCommits });
  } catch (error) {
    console.error('GitHub error:', error.message);
    res.status(500).json({ success: false, error: error.message });
  }
